...
class SubscriptionsScreen extends ConsumerWidget {
                            ...
                            children: [
                              ...
                              PopupMenuButton<String>(
                                onSelected: (value) {
                                  ...
                                  } else if (value == 'edit_items' && isMultiProduct) {
                                    _showEditMultiProductDialog(context, ref, subscription);
                                ...
                                itemBuilder: (context) => [
                                  if (isMultiProduct) ...[
                                    ...
                                    const PopupMenuItem(
                                      value: 'edit_items',
                                      child: Text('Edit Products & Quantities'),
                                    ),
                                  ],
                                  if (!isMultiProduct) ...[
                                    const PopupMenuItem(
  ...
  void _showEditMultiProductDialog(BuildContext context, WidgetRef ref, Subscription subscription) {
    if (subscription.items == null || subscription.items!.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No items found in subscription')),
      );
      return;
    }

    // Create a copy of items with controllers for editing
    final List<Map<String, dynamic>> editableItems = subscription.items!.map((item) {
      return {
        'item': item,
    ...
    showDialog(
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          title: const Text('Edit Products & Quantities'),
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text(
                  'Adjust daily quantities for each product:',
                  style: TextStyle(fontSize: 14, color: Colors.grey),
                ),
                const SizedBox(height: 16),
                ...editableItems.map((itemData) {
                  final item = itemData['item'] as SubscriptionItem;
                  final controller = itemData['controller'] as TextEditingController;
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 8),
                          Text(
                            '₹${item.pricePerUnit.toStringAsFixed(2)} per ${item.unit ?? 'unit'}',
                ...
                if (editableItems.isEmpty)
                  const Padding(
                    ...
                    child: Text(
                      'No products selected. Add at least one product.',
          ...
          actions: [
            TextButton(
              onPressed: () {
                // Dispose controllers
                for (var itemData in editableItems) {
                  (itemData['controller'] as TextEditingController).dispose();
                }
                Navigator.pop(dialogContext);
              },
              child: const Text('Cancel'),
            ...
            ElevatedButton(
              onPressed: editableItems.isEmpty ? null : () async {
                ...
                for (var itemData in editableItems) {
                  ...
                  if (quantity == null || quantity <= 0) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text('Invalid quantity for ${item.productName}')),
                    );
                    return;
                  }
                ...
                Navigator.pop(dialogContext);
                await _updateMultiProductSubscription(
                  context,
                  ref,
                  subscription.id,
                  updatedItems,
  ...
  Future<void> _updateMultiProductSubscription(
    BuildContext context,
    WidgetRef ref,
    String subscriptionId,
    ...
    try {
      final response = await apiService.put(
        '${ApiConfig.subscriptions}/$subscriptionId/items',
        data: {
          'items': items,
        },
      );
      ...
      if (response.data['success']) {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Subscription items updated successfully!'),
